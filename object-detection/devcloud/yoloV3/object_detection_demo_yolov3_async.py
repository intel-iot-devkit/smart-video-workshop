...
def main():
    ...
    yolov3_counter = cv2.VideoWriter(os.path.join("results"+ "/" + plugin.device ,"cars.mp4"),0x00000021, fps, (int(initial_w), int(initial_h)), True)
    result_file = open(os.path.join("results"+ "/" + plugin.device, 'output.txt'), "w")
    pre_infer_file = os.path.join("results"+ "/" + plugin.device, 'pre_progress.txt')
    infer_file = os.path.join("results"+ "/" + plugin.device, 'i_progress.txt')
    post_infer_file = os.path.join("results"+ "/" + plugin.device, 'post_progress.txt')
        
    #----------------------------------------------- 6. Doing inference -----------------------------------------------
    print("To close the application, press 'CTRL+C' or any key with focus on the output window")
    
    post_infer_times = collections.deque()  
    infer_times = collections.deque()
    pre_infer_times = collections.deque()
    time_start = time()
    while cap.isOpened():
        pre_inf_start = time()
        ...
        id_ += 1
        pre_inf_total = time() - pre_inf_start
        pre_infer_times.append(pre_inf_total*1000)
        pre_end = time()  
        # Start inference
        inf_start = time()
        ...
        start_time = time()
        yolov3_counter.write(frame)
        render_time = time() - start_time
        post_infer_times.append(render_time*1000)
        if id_%10 == 0:
            progressUpdate(post_infer_file, time()-time_start, id_, video_len)
        if is_async_mode:
