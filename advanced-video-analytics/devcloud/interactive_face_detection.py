...
def build_argparser():
    ...
    parser.add_argument("-i", "--input",
                        ...
                        type=str)
    #parser.add_argument("-o", "--output", help="Path to the output file", required=True, type=str)
...
def main():
    ...
    initial_w = cap.get(3)
    initial_h = cap.get(4)
    interact_counter = cv2.VideoWriter(os.path.join(args.output_dir, "face.mp4"),0x00000021, fps, (int(initial_w), int(initial_h)), True)
    ...
    while cap.isOpened():
        ...
        if exec_net.requests[cur_request_id].wait(-1) == 0:
            ...
            progressUpdate(infer_file, (sum(infer_times)/1000), framesCounter+1, 1718)
